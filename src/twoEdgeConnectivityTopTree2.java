...
public class twoEdgeConnectivityTopTree2 implements TopTreeInterface {
    ...
    public void combine(Node t) {
        ...
        // update counters
        if (isPath(t)){
            // Boundary verties a from 0'th, b from 1st c is the shared one
            Vertex a;
            Vertex b;
            Vertex c;
    ...
    public void computeCombines(Node n) {
        if (n.isLeaf) {
            clean(n);
        } else if (n.toBeComputed) {
            InternalNode node = (InternalNode) n;
            ArrayList<Node> children = node.children;
            for (Node child: children){
                computeCombines(child);
            }
            clean(n);
        }
    ...
    public void checkUserInfo(Node real, Node copy) {
        twoEdgeConnectivityUserInfo realInfo = (twoEdgeConnectivityUserInfo) real.userInfo;
        twoEdgeConnectivityUserInfo copyInfo = (twoEdgeConnectivityUserInfo) copy.userInfo;
        if (isPoint(real)){
            // Check counters relevant for leaf nodes
            if ((!realInfo.incident3.equals(copyInfo.incident3)) || (!realInfo.size3.equals(copyInfo.size3))){
                //throw new RuntimeException("Information is not equal in point cluster");
            }

        } else {
            // Check counters relevant for internal nodes
            if ((!realInfo.incident4.equals(copyInfo.incident4)) || (!realInfo.size4.equals(copyInfo.size4))){
                //throw new RuntimeException("Information is not equal in path cluster");
            }
        }
    }
    ...
    private int swap(Vertex u, Vertex v){
        expose(u);
        Node c = expose(v);
        Edge f = Tree.adjacencyList[u.id][v.id];
    ...
    public void insert(Vertex u, Vertex v){
        if (u.firstEdge != null && v.firstEdge != null && findRoot(u.firstEdge.userData).equals(findRoot(v.firstEdge.userData))){
            ...
            if (debug){
                System.out.println("Inserted " + u.id + " " +  v.id + " into the cover edges");
            }
            ...
            link(u,v,1);
            if (debug){
                System.out.println("Inserted " + u.id + " " + v.id + " into the spanning tree");
            }
        }
    }
    ...
    public void delete(Vertex u, Vertex v){
        ...
        if (i == -1){
            ...
            i = swap(u, v);
            a = findRoot(u.firstEdge.userData);
            if (a.toBeComputed){
                computeCombines(a);
            }
            uncoverReal(u, v, i);
    ...
    public boolean twoEdgeConnected(Vertex u, Vertex v){
        ...
        if (root2.toBeComputed){
            computeCombines(root2);
        }
    ...
    public void coverReal(Vertex u, Vertex v, int i){
        ...
        if (root.toBeComputed){
            computeCombines(root);
        }
